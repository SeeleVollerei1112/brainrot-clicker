...
function UpgradeShopView.render(role, display_data)
    ...
    for _, item_configuration in ipairs(UpgradeShopConfig.ITEMS) do
        ...
        if card and item_display_data then
            ...
            if card.container then role.set_node_touch_enabled(card.container, enabled) end
            if enabled then
                if card.background then
                    role.set_image_color(card.background, colors.card_ready, math.tofixed(0))
                end
                if card.icon then role.set_ui_opacity(card.icon, to_fixed(opacity_configuration.visible)) end
                if card.name then role.set_label_color(card.name, colors.text_dark, to_fixed(0)) end
                if card.description then
                    role.set_label_color(card.description, colors.text_dark, to_fixed(0))
                end
                if card.price then role.set_label_color(card.price, colors.text_dark, to_fixed(0)) end
                if card.level then role.set_label_color(card.level, colors.text_blue, to_fixed(0)) end
            else
                if card.background then
                    role.set_image_color(card.background, colors.card_locked, math.tofixed(0))
                end
                if card.icon then role.set_ui_opacity(card.icon, to_fixed(opacity_configuration.locked_icon)) end
                if card.name then role.set_label_color(card.name, colors.text_dark, to_fixed(0)) end
                if card.description then
                    role.set_label_color(card.description, colors.text_dark, to_fixed(0))
                end
                if card.price then role.set_label_color(card.price, colors.text_grey, to_fixed(0)) end
                if card.level then role.set_label_color(card.level, colors.text_grey, to_fixed(0)) end
            end
        end
    end
end
