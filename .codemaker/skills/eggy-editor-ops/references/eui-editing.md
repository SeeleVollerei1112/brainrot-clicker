...
```bash
# 按名称查找得分文本节点
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.get_eui_node_ids_by_name('ScoreLabel')"

# 获取节点类型
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.get_eui_node_type(node_id)"
```
...
```powershell
# ✅ 正确示例（父节点字符串，坐标浮点）
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.create_eui_label_node('1405255247', 225.0, 60.0, 300.0, 80.0, 'ScoreLabel', 'Score: 0')"

# ❌ 错误：父节点传整数 0，创建失败
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.create_eui_label_node(0, 225, 60, 300, 80, 'ScoreLabel', 'Score: 0')"
```
...
.codemaker\editor-cli.exe --port 19836 exec "print(EditorAPI.get_all_eui_node_ids())"
# → 记录画布节点 ID 字符串，如 "1405255247"

# 步骤 3：创建文本节点（屏幕中上方，坐标和尺寸必须是浮点数）
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.create_eui_label_node('1405255247', 225.0, 60.0, 300.0, 80.0, 'CountdownLabel', '30')"
# → 返回新节点 ID 字符串，如 "label_001"

# 步骤 4：设置字号为 48
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.set_eui_label_font_size('label_001', 48)"

# 步骤 5：设置文字颜色为黄色
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.set_eui_label_color('label_001', 255, 220, 0, 255)"

# 步骤 6：开启白色描边
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.set_eui_label_outline_enabled('label_001', true)"
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.set_eui_label_outline_color('label_001', 255, 255, 255, 255)"
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.set_eui_label_outline_width('label_001', 2.0)"
```
