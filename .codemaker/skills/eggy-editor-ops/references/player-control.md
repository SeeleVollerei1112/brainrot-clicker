...
exec "EditorAPI.game_execute('print(\"MCP_NO_ROLES\")')"
...
```powershell
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('local roles = GameAPI.get_all_roles();if not roles or #roles == 0 then print(''MCP_ERR|NO_ROLES'') return end;local u = roles[1].get_ctrl_unit();u.set_position(math.Vector3(10, 2, -5));print(''MCP_OK|teleport'')')"
```
...
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('local roles = GameAPI.get_all_roles();if not roles or #roles == 0 then print(''MCP_ERR|NO_ROLES'') return end;local u = roles[1].get_ctrl_unit();u.cmd_move_to_pos(math.Vector3(10, 0, 20), 5);print(''MCP_OK|move'')')"
# 等待 5 秒后再查询位置确认
```
...
```powershell
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('local roles = GameAPI.get_all_roles();if not roles or #roles == 0 then print(''MCP_ERR|NO_ROLES'') return end;local u = roles[1].get_ctrl_unit();u.jump();print(''MCP_OK|jump'')')"
```
...
```powershell
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('LuaAPI.global_send_custom_event(''GAME_START'', nil);print(''MCP_EVENT|OK'')')"
```
...
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('local d = dict();d.set(d, ''coinId'', 42);LuaAPI.global_send_custom_event(''coin_picked'', d);print(''MCP_EVENT|OK'')')"
```
...
# 2. 获取当前位置（确认基准值）
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('..MCP_PLAYER|..')"
# 读取 log.txt 新增行，解析位置

# 3. 瞬移到终点坐标 (100, 2, 50)
.codemaker\editor-cli.exe --port 19836 exec "EditorAPI.game_execute('..set_position(math.Vector3(100, 2, 50))..MCP_OK|teleport')"
